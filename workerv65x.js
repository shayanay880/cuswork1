...
const OVERALL_CONNECTION_TIMEOUT = 120000; // 2 minutes safety timeout to prevent hung sessions
...
async function getDataset(request, env, context) {
  ...
  try {
        ...
        async () => {
          ...
          if (settingsData) {
            try {
              ...
              return normalizeProxySettingsShape(parsed);
            } catch (e) {
              console.error("JSON parse failed for proxySettings, using defaults");
              return normalizeProxySettingsShape(DEFAULT_SETTINGS);
            }
          } else {
            console.log("No proxySettings in KV, initializing with defaults");
            const defaults = normalizeProxySettingsShape(DEFAULT_SETTINGS);
    ...
    // Emergency fallback
    return { 
      settings: normalizeProxySettingsShape(DEFAULT_SETTINGS),
      warpAccounts: []
    };
  }
}
...
async function updateDataset(request, env) {
  ...
  const normalizedSettings = normalizeProxySettingsShape(updatedSettings);
  
  const success = await saveDataset(env, "proxySettings", normalizedSettings);
  if (!success) {
  ...
  CACHE_VERSION++;
  SETTINGS_CACHE.data = normalizedSettings;
...
const DEFAULT_TLS_PORTS = [443, 8443, 2053, 2083, 2087, 2096];
const DEFAULT_HTTP_PORTS = [80, 8080, 8880, 2052, 2082, 2086, 2095];
const ALLOWED_PANEL_PORTS = /* @__PURE__ */ new Set([...DEFAULT_HTTP_PORTS, ...DEFAULT_TLS_PORTS]);
function normalizePorts(ports, fallback = DEFAULT_SETTINGS.ports) {
  const source = Array.isArray(ports) ? ports : fallback;
  const normalized = source.map((value) => Number(value)).filter((port) => Number.isInteger(port) && ALLOWED_PANEL_PORTS.has(port));
  const unique = [...new Set(normalized)];
  return unique.length ? unique : [...fallback];
}
function normalizeProxySettingsShape(settings) {
  const raw = settings && typeof settings === "object" ? settings : {};
  const merged = {
    ...DEFAULT_SETTINGS,
    ...raw
  };
  const fallbackPorts = DEFAULT_SETTINGS.ports;
  const legacyTlsPorts = Array.isArray(raw.tlsPorts) ? raw.tlsPorts : [];
  const legacyHttpPorts = Array.isArray(raw.httpPorts) ? raw.httpPorts : [];
  const combinedLegacyPorts = legacyTlsPorts.length || legacyHttpPorts.length ? [...legacyHttpPorts, ...legacyTlsPorts] : null;
  merged.ports = normalizePorts(raw.ports ?? combinedLegacyPorts, fallbackPorts);
  merged.panelVersion = DEFAULT_SETTINGS.panelVersion;
  return merged;
}
...
async function getSettings(ctx) {
  ...
  const data = {
    proxySettings: normalizeProxySettingsShape(dataset.settings),
    isPassSet,
    subPath
  };
