...
function checkRateLimit(clientIP) {
  ...
  const key = `${clientIP}_${currentMinute}`;
  const count = RATE_LIMIT.get(key) || 0;
  if (count >= MAX_REQUESTS_PER_MINUTE) {
    if (DEBUG_MODE) {
      console.log(`[RATE LIMIT] IP ${clientIP} exceeded ${MAX_REQUESTS_PER_MINUTE} req/min`);
    }
    return false;
  }
...
function checkConnectionLimit(clientIP) {
  if (!clientIP || clientIP === "unknown") return true;
  const count = ACTIVE_CONNECTIONS.get(clientIP) || 0;
  if (count >= MAX_CONNECTIONS_PER_IP) {
    if (DEBUG_MODE) {
      console.log(`[CONN LIMIT] IP ${clientIP} has ${count} connections (max ${MAX_CONNECTIONS_PER_IP})`);
    }
    return false;
  }
  return true;
...
function trackConnection(clientIP) {
  if (!clientIP || clientIP === "unknown") return () => {};
  const count = ACTIVE_CONNECTIONS.get(clientIP) || 0;
  ACTIVE_CONNECTIONS.set(clientIP, count + 1);
  return () => {
    const current = ACTIVE_CONNECTIONS.get(clientIP) || 0;
    if (current <= 1) {
      ACTIVE_CONNECTIONS.delete(clientIP);
    } else {
      ACTIVE_CONNECTIONS.set(clientIP, current - 1);
    }
  };
}
...
async function safeWriteToOutbound(remoteSocketWapper, data) {
  ...
  const writePromise = remoteSocketWapper.writeChain.then(async () => {
    ...
    if (!remoteSocketWapper.value.writable) {
      throw new Error("Socket has no writable stream");
    }
    let writer;
    try {
      ...
      if (lowerMsg.includes("closed") || lowerMsg.includes("closing") || lowerMsg.includes("aborted") || lowerMsg.includes("belongs to an object that is closing")) {
        remoteSocketWapper.value = null;
        if (DEBUG_MODE) {
          console.log("Stream already closing, gracefully exiting");
        }
        return;
      }
      throw error;
...
var worker_default = {
  async fetch(request, env, context) {
    ...
    if (isWebSocket) {
      ...
      const cleanup = trackConnection(clientIP);
    ...
    const isStreamEndpoint = pathname.includes("/sub/") || pathname.includes("/vless") || pathname.includes("/trojan") || pathname === "/" && isWebSocket;

    if (isWebSocket || isStreamEndpoint) {
      if (DEBUG_MODE) {
        console.log(`[DEDUP] Skipping dedup for stream/WS request: ${pathname}`);
      }
      return await runWithTimeout().catch(async (error) => {
        const message2 = error instanceof Error ? error.message : String(error);
