...
// workerv62x.js — optimized build based on workerv61x
// Changes vs v61x:
//
// CRITICAL FIXES (bugs / connection slot leaks):
//   C1. resolveDNS: Check DNS_IN_FLIGHT BEFORE deleting stale cache entry —
//       old code deleted valid fallback data before it could be served as a fallback.
//   C2. convertToNAT64IPv6: Explicit throw on bad prefix format instead of returning undefined —
//       undefined hostname caused silent 3s timeout per affected connection.
//   C3. extractProxyParams: try/catch around new URL() — malformed chain proxy no longer throws.
//   C4. Cancel unread DoH response bodies on !ok — frees outbound connection slot immediately
//       (both handleUDPOutBound and fetchDNSRecords paths).
//   C5. getCachedTrojanPasswordHash: Removed async/await — sha224 is synchronous; was adding
//       a useless microtask tick on every Trojan auth check.
//
// HOT PATH (latency + upstream VPN):
//   H1. Backpressure loop: module-level _sleep() avoids Promise+closure alloc per tick;
//       added safety guard for infinite-loop edge case (single chunk > queue limit).
//   H3. updateDataset: write fresh data directly into SETTINGS_CACHE (no KV re-read);
//       CACHE_VERSION incremented inline — WARP_CACHE data preserved (settings ≠ WARP).
//   H4. ETag/304 support on /sub/* — upstream VPN polling gets zero-body 304 when configs unchanged.
//   H5. Event-loop yield every 50 iterations in getXrCustomConfigs and getSbCustomConfig
//       (converted getSbCustomConfig.forEach loops to async for-of) — prevents VPN tunnel
//       ping spikes during config generation.
//   H6. customCdnAddrs converted to Set after settings load — O(1) .has() vs O(N) .includes()
//       for every URI generated; updated all 3 call sites.
//   H7. buildConfig: getGeoAssets computed once, passed to buildDNS/buildRoutingRules/buildRuleProviders.
//       All three functions updated to accept pre-computed geoAssets (with fallback for other callers).
//   H8. safeWriteToOutbound: console.error gated behind env.DEBUG flag — no CPU cost in production.
//
// MEDIUM:
//   M1. VlOverWSHandler: removed VLVersion from parseVlHeader destructure — field was never read.
//   M2. remoteSocketToWS: removed dead instanceof Uint8Array check — CF TCP always delivers Uint8Array.
//   M3. handleWebsocket catch: collapsed identical if/else branches into single unconditional fallback.
//
// LOW / NEW:
//   L4. setDNSCacheEntry: removed redundant .has() before .delete() — Map.delete() is safe on miss.
//   N1. makeReadableWebSocketStream: controller.error() now passes string not new Error() —
//       avoids V8 stack trace capture in the hot inbound stream path.
//   N2. handleUDPOutBound / fetchDNSRecords: +contentLength instead of Number.parseInt(x, 10).
//   N3. handleTCPOutBound retry: static 100ms replaces Math.pow + Math.random jitter (single-user).
//
// Inherited from v61x (unchanged):
//   L2. sha224: TextEncoder.encode() instead of manual utf8Encode loop.
//   L3. sha224: Uint32Array(64) hoisted outside chunk loop.
//   L5. omitEmpty: for-in instead of Object.keys().length === 0.
//   L1. OVERALL_CONNECTION_TIMEOUT=0 → createOverallConnectionTimeout is a no-op stub.
...
async function buildDNS(ctx, isChain, isWarp, isPro, geoAssets) {
  ...
  const geoAssets = geoAssets ?? getGeoAssets(ctx);  // H7: prefer pre-computed from buildConfig
...
function parseVlHeader(VLBuffer, userID) {
  ...
  // D33: version byte read directly — 1-element array, no slice
