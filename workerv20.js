...
const REQUEST_DEDUP = new Map(); // Global map for deduplicating identical concurrent requests
...
async function resolveDNS(domain, onlyIPv4 = false, dohURL = "https://cloudflare-dns.com/dns-query") {
  ...
  if (cached && Date.now() - cached.timestamp < DNS_CACHE_TTL) {
    // True LRU: Refresh entry position
    DNS_CACHE.delete(cacheKey);
    DNS_CACHE.set(cacheKey, cached);
    return cached.data;
  }
  
  // Request Coalescing: Return existing promise if already in flight
  ...
  const fetchPromise = (async () => {
    try {
      const DNS_TIMEOUT = 3000; // 3 seconds timeout for DNS

      // Add request timeout wrapper
      const fetchWithTimeout = (url, recordType, timeout) => {
        return Promise.race([
          fetchDNSRecords(url, recordType),
      ...
      DNS_CACHE.set(cacheKey, { data: result, timestamp: Date.now() });
      return result;
    } catch (error) {
...
async function handleTCPOutBound(ctx, remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, VLResponseHeader, log) {
  const {
    proxyMode,
    panelIPs,
    envProxyIPs,
  ...
  for (let i = 0; i <= MAX_RETRIES; i++) {
    ...
    try {
      ...
      log(`connected to ${currentAddress}:${currentPort} (attempt ${i + 1})`);
    ...
    try {
      ...
      remoteSocketToWS(tcpSocket, webSocket, VLResponseHeader, log).finally(() => {
        safeCloseWebSocket(webSocket);
      });
...
async function remoteSocketToWS(remoteSocket, webSocket, VLResponseHeader, log) {
...
async function safeWriteToOutbound(remoteSocketWapper, data) {
  ...
  while (remoteSocketWapper.isWriting) {
    if (Date.now() - startTime > MAX_WAIT) {
      throw new Error("Write timeout - socket may be stuck");
    }
    if (!remoteSocketWapper.value) {
      throw new Error("Socket closed while waiting to write");
    }
  ...
  remoteSocketWapper.isWriting = true;
  ...
  try {
    ...
    console.error("safeWriteToOutbound write error:", error.message);
    ...
    if (writer) {
      try {
        ...
        console.error("Error releasing writer lock:", e.message);
      }
    }
    remoteSocketWapper.isWriting = false;
  }
}
...
async function VlOverWSHandler(ctx) {
  ...
  console.error(`DEBUG: VlOverWSHandler starting. userID="${userID}"`);
  ...
  let remoteSocketWapper = { value: null, connecting: false, isWriting: false };
...
// Helper Functions
// isValidUUID removed (duplicate)
// renderError removed (duplicate)

// sha224 removed (duplicate)
...
var worker_default = {
  async fetch(request, env, context) {
    ...
    // Request Deduplication: If an identical request is already being processed, share the promise
    if (REQUEST_DEDUP.has(cacheKey)) {
      console.log(`[DEDUP] Coalescing request: ${cacheKey}`);
      return REQUEST_DEDUP.get(cacheKey);
    }
    ...
    const fetchPromise = (async () => {
      try {
        ...
        // Clear from dedup map as soon as request finishes
        REQUEST_DEDUP.delete(cacheKey);
      }
    })().catch(async (error) => {
    ...
    REQUEST_DEDUP.set(cacheKey, fetchPromise);
    return fetchPromise;
  },
