...
const CONNECTION_LEASE_MS = 300000; // 5 minutes soft lease for websocket sessions
let connectionCleanupCounter = 0;
...
function pruneConnectionLeases(now = Date.now()) {
  for (const [ip, leases] of ACTIVE_CONNECTIONS.entries()) {
    const alive = leases.filter((expiresAt) => expiresAt > now);
    if (alive.length === 0) {
      ACTIVE_CONNECTIONS.delete(ip);
    } else {
      ACTIVE_CONNECTIONS.set(ip, alive);
    }
  }
}
...
function checkConnectionLimit(clientIP) {
  const ipKey = normalizeClientIP(clientIP);
  if (!ipKey || ipKey === "unknown") return true;
  pruneConnectionLeases();
  const leases = ACTIVE_CONNECTIONS.get(ipKey) || [];
  if (leases.length >= MAX_CONNECTIONS_PER_IP) {
    if (DEBUG_MODE) {
      console.log(`[CONN LIMIT] IP ${ipKey} has ${leases.length} leased connections (max ${MAX_CONNECTIONS_PER_IP})`);
    }
    return false;
  }
  return true;
...
function trackConnection(clientIP) {
  ...
  const now = Date.now();
  const leases = ACTIVE_CONNECTIONS.get(ipKey) || [];
  leases.push(now + CONNECTION_LEASE_MS);
  ACTIVE_CONNECTIONS.set(ipKey, leases);

  connectionCleanupCounter++;
  if (connectionCleanupCounter >= RATE_LIMIT_CLEANUP_INTERVAL) {
    connectionCleanupCounter = 0;
    pruneConnectionLeases(now);
  }
}
  ...
  const writePromise = remoteSocketWapper.writeChain.then(async () => {
    ...
    try {
      ...
      if (lowerMsg.includes("closed") || lowerMsg.includes("closing") || lowerMsg.includes("aborted") || lowerMsg.includes("belongs to an object that is closing")) {
        ...
        throw new Error("Socket closing during write");
      }
      throw error;
    } finally {
      if (writer) {
...
var worker_default = {
  async fetch(request, env, context) {
    ...
    if (!isWebSocket && !checkRateLimit(clientIP)) {
