  ...
  const fetchPromise = (async () => {
    try {
      ...
      const results = await Promise.allSettled([
        withTimeout(fetchDNSRecords(dohURLs.ipv4, 1), DNS_TIMEOUT, "IPv4 DNS timeout"),
        onlyIPv4 ? Promise.resolve([]) : withTimeout(fetchDNSRecords(dohURLs.ipv6, 28), DNS_TIMEOUT, "IPv6 DNS timeout")
      ]);
