...
// workerv61x.js — hardened build based on workerv60x
// Changes vs v60x:
//   1. FIX FATAL: WARP crash guard — all three WARP config functions (getClWarpConfig,
//      getSbWarpConfig, getXrWarpConfigs) now return HTTP 503 with a human-readable
//      message when warpAccounts is empty (e.g. upstream 429 rate-limit), instead of
//      crashing with TypeError: Cannot destructure property of undefined.
//   2. FIX MEDIUM: Removed webSocket.ping() from createIdleWatchdog — this method does
//      not exist in the Cloudflare Workers WebSocket API. Every call silently threw a
//      TypeError caught by the surrounding catch block, wasting CPU cycles with zero
//      keepalive benefit. The CF runtime handles WS ping/pong at the protocol level
//      automatically.
//   3. FIX MEDIUM: DNS resolution changed from simultaneous Promise.any() racing to
//      sequential primary-then-fallback. Racing abandoned the losing DNS fetch while it
//      kept running, wasting one of the 6 simultaneous outbound connection slots that
//      the Workers runtime allows per invocation — a real constraint for a proxy/VPN
//      worker that already holds an inbound WS + outbound TCP socket.
...
async function parseTrHeader(ctx, buffer) {
  ...
  if (password !== await getCachedTrojanPasswordHash(TrPass)) {
    return {
      hasError: true,
